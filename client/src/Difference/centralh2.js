...
class Centralh2 extends Component {
    ...
    centralh2(x,h,deg) {
        switch(deg){
            case 1 :
                y= (-this.fx(x+(2*h))+8*this.fx(x+h)-8*this.fx(x-h)+this.fx(x-(2*h)))/(12*h);
               break;
            case 2 :
                y= (-this.fx(x+(2*h))+(16*this.fx(x+h))-30*this.fx(x)+16*this.fx(x-h)-this.fx(x-(2*h)))/(12*Math.pow(h,2));
               break;
            case 3 :
                y = (-this.fx(x+(3*h))+8*this.fx(x+(2*h))-(13*this.fx(x+h))+(13*this.fx(x-h))-8*this.fx(x-(2*h))+this.fx(x-(3*h)))/(8*Math.pow(h,3));
                break;
            default :
                y = (-this.fx(x+(3*h))+12*this.fx(x+(2*h))-(39*this.fx(x+h))+(56*this.fx(x))-(39*this.fx(x-h))+12*this.fx(x-(2*h)-this.fx(x-(3*h))))/(6*Math.pow(h,4));
                break;
       }
        this.setState({
            showAnswer: true
    ...
    render() {
        return (
            <div className="Centralh2" >
                <h1>Central Divided-differences ( Error of order O(h))</h1>
                <div style={{ padding: "50px 200px 200px" }}>                
                    ...
                    <Button variant="outline-primary" onClick={() => this.centralh2(parseFloat(this.state.x), parseFloat(this.state.h),parseInt(this.state.deg))}>Submit</Button>
                    <br /><br />
                    </div>
