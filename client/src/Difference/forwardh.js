    ...
    forwardh(x,h,deg) {
        switch(deg){
            case 1 :
               ...
               break;
            case 2 :
                y= (this.fx(x+(2*h))-(2*this.fx(x+h))+this.fx(x))/Math.pow(h,2);
               break;
            case 3 :
                y = (this.fx(x+(3*h))-(3*this.fx(x+(2*h)))+(3*this.fx(x+h))-this.fx(x))/Math.pow(h,3);
                break;
            default :
                y = (this.fx(x+(4*h))-(4*this.fx(x+(3*h)))+(6*this.fx(x+(2*h)))-(4*this.fx(x+h))+this.fx(x))/Math.pow(h,4);
                break;
       }
        this.setState({
            showAnswer: true
    ...
    render() {
        return (
            <div className="Forwardh" >
                <h1>Forward Divided-differences ( Error of order h)</h1>
                <div style={{ padding: "50px 200px 200px" }}>                
                    ...
                    <Button variant="outline-primary" onClick={() => this.forwardh(parseFloat(this.state.x), parseFloat(this.state.h),parseInt(this.state.deg))}>Submit</Button>
                    <br /><br />
                    </div>
